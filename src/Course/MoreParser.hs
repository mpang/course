...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok f s = between (charTok f) (charTok s)
...
hex ::
  Parser Char
hex = do
  result <- replicateA 4 $ satisfy isHexDigit
  case readHex result of
    Empty  -> failed
    Full r -> return $ chr r
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby f s = sepby1 f s ||| valueParser Nil
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll predicates = do
  c <- character
  if and $ sequence predicates c then valueParser c else failed
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny predicates = do
  c <- character
  if or $ sequence predicates c then valueParser c else failed
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma f s p = betweenCharTok f s $ sepby p $ charTok ','
