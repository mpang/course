...
filtering ::
  ...
  -> List a
  -> f (List a)
-- used foldRight to accumulate list
filtering predicate = foldRight helper (pure Nil)
  -- basically lift a function with parameter types (bool -> List a) to ((f Bool) -> f (List a))
  where helper x = lift2 (\result list -> if result then x :. list else list) $ predicate x
