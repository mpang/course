...
convertInteractive ::
  IO ()
convertInteractive =
  putStr "Enter a string to be converted to upper case: " >-
...
reverseInteractive =
  ...
  getLine >>= \destination ->
  readFile source >>= \content -> writeFile destination (reverse content) >-
  putStrLn "Done"
...
encodeInteractive =
  ...
  getLine >>= \string -> putStrLn $ convert string
  where
    convert :: Chars -> Chars
    convert string =
      string >>= \c -> case c of
